...
public class TicketTypeController {
	...
	def update = {
		...
		if(ticketType && product.company == company){
			...
			withFormat {
				html ticketType:ticketType
				xml {  render ticketType as XML }
				json { render ajaxResponseService.prepareResponse(ticketType, ticketType.asMapForJSON()).asMap() as JSON }
			}
		}
		else {
			response.sendError 404
	...
	private boolean updateSockCalendar(TicketType ticketType,Long stockVariation, ProductCalendar calendarType) {
		...
		def stockCalendars = StockCalendar.createCriteria().list {
			...
			if (calendarType!=ProductCalendar.NO_DATE) { ge('startDate',today) }
		}
